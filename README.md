...
```
3. For LLM Ollama locally from https://ollama.com/
Then installed the model that I used (llama3.2) by typing in terminal
```bash
ollama pull llama3.2
```
4. Finally execute `scripts/init.py` for the first time only once to get some historical OT data.

5. To make schedule script to run every 10 min to get the latest data, just type (For windows):
```bash
./run.ps1
```
...
## 🧠 Prompt/logic used
In this section I will talk about how I desgined the prompt for the LLM model to be able to produce accurate results:
1. I assigned it a role to be as an OT expert to analyze the vulnerabilities.
```
...
4. Gave the model all the data it needs or help for analysis, the data includes:
  - CVE-ID (So that if the model is fine-tuned on new data, it can gives rich information, better if the LLM model operates on the cloud).
  - Description.
  - Metrics (related to risks and impacte, hepled alot), Ex:
```
Vulnerability Data:
    - CVE-ID: {cve_id}, 
...
6. If one of metrics' details was null I filled it will `None` not `0` for example as this will affect the model reasoning.

7. The model is loaded one at the begining at operates separatly on each record in the loop.
...
In the first place I read the NVD API documentation on: https://nvd.nist.gov/developers/vulnerabilities, and got API key.

It is important to note that there are 3 different schema version, as this affected parsing the data:
...
- `keywordSearch`: returns only the CVEs where a word or phrase is found in the current description. (No OR optionm AND only).
...
This is information which helped me.

---

### Fetching the data and prefiltering
As mentioned we can use `keywordSearch` to get only CVEs that contains specific word.

As there is not `OR` option I created a loop to go through each keyword and make a separate `GET` request, then combine all.
...
This is extermly faster and more optimized than getting all the data then apply regex for example or keyword search on the data. Also the usage of the API params gives more reliable and accurate results.

This also better than pushing large data to the LLM model, will consume huge resources.

##### URL params
- `pubEndDate`: Default is now, can be changed.
...
The prompt logic was described above and the application of second filtering also.
...
It is in the form of python dictionary and the keys is CVE-ID to ensure that there will not be any repeated data, and also to take the advantage of hashtable.

---

### Streamlit Dashboard
There is 2 main pages:
...
- Easy filtering by: date, search by CVE-ID or describtion.
- Auto refresh every 10 min.
- A button to refresh by manually.
